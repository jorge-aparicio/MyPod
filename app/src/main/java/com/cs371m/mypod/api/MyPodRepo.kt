...
class MyPodRepo(private val podchaserAPI: ApolloClient) {
    ...
    // Search for podcasts
    suspend fun podcastSearch(searchTerm: String, limit: Int): ApolloResponse<PodcastSearchQuery.Data> {
        Log.d("#########################", "${podchaserAPI.query(PodcastSearchQuery(limit, searchTerm)).execute().data!!.podcasts}")
        return podchaserAPI.query(PodcastSearchQuery(limit, searchTerm)).execute();
    }

    // Get profile details
    suspend fun getProfile(id: String): ApolloResponse<ProfileQuery.Data> {
        return podchaserAPI.query(ProfileQuery(id)).execute();
    }
